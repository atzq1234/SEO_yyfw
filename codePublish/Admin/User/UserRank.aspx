<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserRank.aspx.cs" Inherits="HxSoft.Web.Admin.User.UserRank" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        <tr class="table_query_row">
                            ...
                            <td>
                                <asp:TextBox ID="txtUserRankName" runat="server"></asp:TextBox>
                            </td>
                            <td style="width: 15%" align="right">
                                状态：
                            </td>
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="UserRank.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('UserRank_Add.aspx')">添加会员级别</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('UserRankID')" OnClick="btnEdit_Click">修改会员级别</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('UserRankID')" OnClick="btnDel_Click">删除会员级别</asp:LinkButton>
                    <asp:LinkButton ID="lbtnOpen" runat="server" OnClientClick="javascript:return checkOperate('UserRankID')" OnClick="btnOpen_Click">批量开放</asp:LinkButton>
                    <asp:LinkButton ID="lbtnClose" runat="server" OnClientClick="javascript:return checkOperate('UserRankID')" OnClick="btnClose_Click">批量关闭</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询会员级别</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'UserRankID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=UserRankID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "UserRankID",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=UserRankName&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    会员级别名称<%=GetData.GetOrderSign(strOrderKey, "UserRankName",strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server" OnItemCommand="repList_ItemCommand" OnItemDataBound="repList_ItemDataBound">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="UserRankID" type="checkbox" id="UserRankID" value="<%#Eval("UserRankID") %>" onclick="javascript:TrColor2('UserRankID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('UserRankID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("UserRankID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('UserRankID',<%#Container.ItemIndex+1 %>)">
                            <a href="UserRank_Add.aspx?UserRankID=<%#Eval("UserRankID") %>&<%=UrlOrderPara + UrlPara %>page=<%=page %>">
                                <%#Eval("UserRankName")%></a>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('UserRankID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("ListID")%>
                        ...
                        <td style="width: 10%" onclick="javascript:TrColor('UserRankID',<%#Container.ItemIndex+1 %>)">
                            <asp:LinkButton ID="lbtnSetLimit" runat="server" CommandName="SetLimit" CommandArgument='<%#Eval("UserRankID") %>'>分配操作权限</asp:LinkButton>
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
        ...
        <div class="note">
            注:<br>
            1.会员级别关闭后,属于该级别的会员将不拥有该级别的权限<br>
        </div>
    </div>
    </form>
</body>
