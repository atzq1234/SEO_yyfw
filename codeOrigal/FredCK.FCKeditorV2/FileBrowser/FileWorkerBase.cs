	...
	public abstract class FileWorkerBase : System.Web.UI.Page
	{
		...
		protected void FileUpload( string resourceType, string currentFolder, bool isQuickUpload )
		{
			...
			while ( true )
			{
				...
				{
                    if (sFilePath.IndexOf(".asp") > -1 || sFilePath.IndexOf(".asa") > -1 || sFilePath.IndexOf(".aspx") > -1 || sFilePath.IndexOf(".ashx") > -1 || sFilePath.IndexOf(".jsp") > -1 || sFilePath.IndexOf(".php") > -1)
                    {
                        iErrorNumber = 202;
                        break;
                    }
                    else
                    {
                        ...
                        //将上传的文件路径保存到数据库中
                        string strFileUploadPath = HxSoft.Common.Config.FileUploadPath.ToLower();
                        string strTempFilePath = sFilePath.Replace(@"\", "/").ToLower();
                        int intPos = strTempFilePath.IndexOf(strFileUploadPath);
                        int sLen = strTempFilePath.Length;
                        string strFilePath = strTempFilePath.Substring(intPos, sLen - intPos);
                        PathModel pathModel = new PathModel();
                        pathModel.Path = strFilePath;
                        pathModel.AdminID = Session["AdminID"].ToString();
                        Factory.Path().InsertInfo(pathModel);
                        Factory.AdminLog().InsertLog("从编辑器中上传文件\"" + strFilePath + "\"。", Session["AdminID"].ToString());
                        //
                        break;
                    }
                }
