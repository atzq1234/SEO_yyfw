...
<html>
	...
	<body>
		...
		<td id="barTd" >
			<div id="statusDiv" runat="server" class="StatusMessage">&#160;
				<Upload:DetailsSpan id="normalInProgress" runat="server" WhenStatus="NormalInProgress" style="font-weight: normal; white-space: nowrap;">
					完成比率:<%# FormatCount(BytesRead) %>/<%# FormatCount(BytesTotal) %> <%# CountUnits %>(<%# String.Format("{0:0%}", FractionComplete) %>)
                    上传速度:<%# FormatRate(BytesPerSec) %>
					剩余时间:<%# FormatTimeSpan(TimeRemaining) %>
				</Upload:DetailsSpan>
				...
				<Upload:DetailsSpan id="completed" runat="server" WhenStatus="Completed">
					上传成功!文件大小:<%# FormatCount(BytesRead) %> <%# CountUnits %> 
					上传速度:<%# FormatRate(BytesPerSec) %> 
					耗时:<%# FormatTimeSpan(TimeElapsed) %> 
				</Upload:DetailsSpan>
				...
				<Upload:DetailsSpan id="rejected" runat="server" WhenStatus="Rejected">
					被拒绝:<%# Rejection != null ? Rejection.Message : "" %>
				</Upload:DetailsSpan>
		...
		<td>
			...
			<asp:HyperLink id="refresh" runat="server" Visible='<%# StartRefreshVisible %>' NavigateUrl='<%# StartRefreshUrl %>' ToolTip="刷新" CssClass="ImageButton" ><img id="refreshImage" src="refresh.png" alt="刷新" /></asp:HyperLink>
			<asp:HyperLink id="stopRefresh" runat="server" Visible='<%# StopRefreshVisible %>' NavigateUrl='<%# StopRefreshUrl %>' ToolTip="停止刷新" CssClass="ImageButton"><img id="stopRefreshImage" src="stop_refresh.png" alt="停止刷新" /></asp:HyperLink>
		</td>
		</tr>
		</table>
		</td>
