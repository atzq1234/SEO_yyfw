    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="Dictionary.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('Dictionary_Add.aspx')">添加字典</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('DictionaryID')" OnClick="btnEdit_Click">修改字典</asp:LinkButton>
                    <asp:LinkButton ID="lbtnMove" runat="server" OnClientClick="javascript:return checkOperate('DictionaryID')" OnClick="lbtnMove_Click">移动字典</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('DictionaryID')" OnClick="btnDel_Click">删除字典</asp:LinkButton>
                    <asp:LinkButton ID="lbtnOpen" runat="server" OnClientClick="javascript:return checkOperate('DictionaryID')" OnClick="btnOpen_Click">批量开放</asp:LinkButton>
                    <asp:LinkButton ID="lbtnClose" runat="server" OnClientClick="javascript:return checkOperate('DictionaryID')" OnClick="btnClose_Click">批量关闭</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询字典</a>
                    <asp:LinkButton ID="lbtnGoBack" runat="server" OnClientClick="javascript:return GoTo('Dictionary.aspx')">返回上级</asp:LinkButton>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                ...
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=DictionaryID&AscDesc=<%=strAscDesc2 %>&ParentID=<%=ParentID %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "DictionaryID",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=DictionaryName&AscDesc=<%=strAscDesc2 %>&ParentID=<%=ParentID %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    字典名称<%=GetData.GetOrderSign(strOrderKey, "DictionaryName", strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=DictionaryVal&AscDesc=<%=strAscDesc2 %>&ParentID=<%=ParentID %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    字典值<%=GetData.GetOrderSign(strOrderKey, "DictionaryVal", strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        ...
                        <td style="width: 10%">
                            <a href="Dictionary.aspx?ParentID=<%#Eval("DictionaryID") %>">进入子字典</a>
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
