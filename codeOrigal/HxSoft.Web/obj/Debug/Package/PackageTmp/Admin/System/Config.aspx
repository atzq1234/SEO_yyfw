<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Config.aspx.cs" Inherits="HxSoft.Web.Admin._System._Config" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="Config.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('Config_Add.aspx')">添加网站配置</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('ConfigID')" OnClick="btnEdit_Click">修改网站配置</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('ConfigID')" OnClick="btnDel_Click">删除网站配置</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询网站配置</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'ConfigID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=ConfigID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "ConfigID",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=LanguageVer&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    语言版本<%=GetData.GetOrderSign(strOrderKey, "LanguageVer", strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=WebsiteName&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    网站名称<%=GetData.GetOrderSign(strOrderKey, "WebsiteName",strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="ConfigID" type="checkbox" id="ConfigID" value="<%#Eval("ConfigID") %>" onclick="javascript:TrColor2('ConfigID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('ConfigID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("ConfigID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('ConfigID',<%#Container.ItemIndex+1 %>)">
                            <a href="Config_Add.aspx?ConfigID=<%#Eval("ConfigID") %>&<%=UrlOrderPara + UrlPara %>page=<%=page %>">
                                <%#Eval("LanguageVer")%>
                            </a>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('ConfigID',<%#Container.ItemIndex+1 %>)">
