<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserLog.aspx.cs" Inherits="HxSoft.Web.Admin.User.UserLog" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td colspan="3">
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="UserLog.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('UserlogID')" OnClick="btnDel_Click">删除会员日志</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询会员日志</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'UserlogID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=UserlogID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "UserlogID",strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="UserlogID" type="checkbox" id="UserlogID" value="<%#Eval("UserlogID") %>" onclick="javascript:TrColor2('UserlogID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('UserlogID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("UserlogID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('UserlogID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("LogContent")%>
