<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Survey.aspx.cs" Inherits="HxSoft.Web.Admin.Survey.Survey" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="Survey.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('Survey_Add.aspx')">添加调查</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('SurveyID')" OnClick="lbtnEdit_Click">修改调查</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('SurveyID')" OnClick="btnDel_Click">删除调查</asp:LinkButton>
                    <asp:LinkButton ID="lbtnOpen" runat="server" OnClientClick="javascript:return checkOperate('SurveyID')" OnClick="btnOpen_Click">批量开放</asp:LinkButton>
                    <asp:LinkButton ID="lbtnClose" runat="server" OnClientClick="javascript:return checkOperate('SurveyID')" OnClick="btnClose_Click">批量关闭</asp:LinkButton>
                    <asp:LinkButton ID="lbtnTransfer" runat="server" OnClientClick="javascript:return checkOperate('SurveyID')" OnClick="btnTransfer_Click">转移调查</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询调查</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'SurveyID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=SurveyID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "SurveyID",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=Subject&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    调查主题<%=GetData.GetOrderSign(strOrderKey, "Subject",strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="SurveyID" type="checkbox" id="SurveyID" value="<%#Eval("SurveyID") %>" onclick="javascript:TrColor2('SurveyID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('SurveyID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("SurveyID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('SurveyID',<%#Container.ItemIndex+1 %>)">
                            <a href="Survey_Add.aspx?SurveyID=<%#Eval("SurveyID") %>&<%=UrlOrderPara + UrlPara %>page=<%=page %>">
                            <%#Eval("Subject")%>
                            </a><%#GetData.ShowRecommendStatus(Eval("IsRecommend").ToString())%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('SurveyID',<%#Container.ItemIndex+1 %>)">
                        ...
                        <td style="width: 10%" onclick="javascript:TrColor('SurveyID',<%#Container.ItemIndex+1 %>)">
                            <a href="SurveyItem.aspx?SurveyID=<%#Eval("SurveyID") %>&ProductPage=<%=page %>">管理调查选项</a> | 
                            <a href="SurveyResult.aspx?SurveyID=<%#Eval("SurveyID") %>&ProductPage=<%=page %>">查看调查结果</a>
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
