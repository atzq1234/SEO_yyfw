<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Link.aspx.cs" Inherits="HxSoft.Web.Admin.Extension.Link" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="Link.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('Link_Add.aspx')">添加友情链接</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('LinkID')" OnClick="btnEdit_Click">修改友情链接</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('LinkID')" OnClick="btnDel_Click">删除友情链接</asp:LinkButton>
                    <asp:LinkButton ID="lbtnOpen" runat="server" OnClientClick="javascript:return checkOperate('LinkID')" OnClick="btnOpen_Click">批量开放</asp:LinkButton>
                    <asp:LinkButton ID="lbtnClose" runat="server" OnClientClick="javascript:return checkOperate('LinkID')" OnClick="btnClose_Click">批量关闭</asp:LinkButton>
                    <asp:LinkButton ID="lbtnTransfer" runat="server" OnClientClick="javascript:return checkOperate('LinkID')" OnClick="btnTransfer_Click">转移友情链接</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询友情链接</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'LinkID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=LinkID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "LinkID",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=SiteName&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    网站名称/Logo<%=GetData.GetOrderSign(strOrderKey, "SiteName",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=SiteUrl&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    网站地址<%=GetData.GetOrderSign(strOrderKey, "SiteUrl",strAscDesc1)%>
                </td>
                ...
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=TypeID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    类型<%=GetData.GetOrderSign(strOrderKey, "TypeID",strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="LinkID" type="checkbox" id="LinkID" value="<%#Eval("LinkID") %>" onclick="javascript:TrColor2('LinkID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('LinkID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("LinkID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('LinkID',<%#Container.ItemIndex+1 %>)">
                            <a href="Link_Add.aspx?LinkID=<%#Eval("LinkID") %>&<%=UrlOrderPara + UrlPara %>page=<%=page %>" title="<%#Eval("SiteName")%>">
                                <%#Eval("TypeID").ToString() == "2" ? "<img src=\""+Eval("LogoUrl")+"\" width=\"88\" height=\"31\"" : Eval("SiteName")%>
                            </a>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('LinkID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("SiteUrl")%>
                        ...
                        <td style="width: 10%" onclick="javascript:TrColor('LinkID',<%#Container.ItemIndex+1 %>)">
                            <%#GetData.ShowLinkType(Eval("TypeID").ToString())%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('LinkID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("ListID")%>
                        </td>
