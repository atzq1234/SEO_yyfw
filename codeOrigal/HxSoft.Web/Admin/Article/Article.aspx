<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Article.aspx.cs" Inherits="HxSoft.Web.Admin.Article.Article" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="Article.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('Article_Add.aspx')">添加文章</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('ArticleID')" OnClick="btnEdit_Click">修改文章</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('ArticleID')" OnClick="btnDel_Click">删除文章</asp:LinkButton>
                    <asp:LinkButton ID="lbtnOpen" runat="server" OnClientClick="javascript:return checkOperate('ArticleID')" OnClick="btnOpen_Click">批量开放</asp:LinkButton>
                    <asp:LinkButton ID="lbtnClose" runat="server" OnClientClick="javascript:return checkOperate('ArticleID')" OnClick="btnClose_Click">批量关闭</asp:LinkButton>
                    <asp:LinkButton ID="lbtnTransfer" runat="server" OnClientClick="javascript:return checkOperate('ArticleID')" OnClick="btnTransfer_Click">转移文章</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询文章</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'ArticleID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=ArticleID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    编号<%=GetData.GetOrderSign(strOrderKey, "ArticleID",strAscDesc1)%>
                </td>
                ...
                <%if (GetData.LimitChk("ArticlePic"))
                  { %>
                <td style="width: 10%">
                    更多图片
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="ArticleID" type="checkbox" id="ArticleID" value="<%#Eval("ArticleID") %>" onclick="javascript:TrColor2('ArticleID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('ArticleID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("ArticleID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('ArticleID',<%#Container.ItemIndex+1 %>)" align="left">
                            <a href="Article_Add.aspx?ArticleID=<%#Eval("ArticleID") %>&<%=UrlOrderPara + UrlPara %>page=<%=page %>">
                                <%#Eval("Title")%></a><%#GetData.ShowRecommendStatus(Eval("IsRecommend").ToString())%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('ArticleID',<%#Container.ItemIndex+1 %>)">
                        ...
                        <%if (GetData.LimitChk("ArticlePic"))
                          { %>
                        <td style="width: 10%">
                            <a href="ArticlePic.aspx?ArticleID=<%#Eval("ArticleID") %>&ArticlePage=<%=page %>">管理</a>
                        </td>
                        <%} %>
                    </tr>
                </ItemTemplate>
