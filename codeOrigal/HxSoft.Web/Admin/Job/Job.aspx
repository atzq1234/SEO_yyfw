<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Job.aspx.cs" Inherits="HxSoft.Web.Admin.Job.Job" %>
    ...
    <div id="container">
                ...
                <div class="content">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_query">
                        ...
                        <tr class="table_query_btn">
                            ...
                            <td>
                                <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Text="查询" PostBackUrl="Job.aspx" />
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_op">
            <tr>
                <td>
                    操作选项：
                    <asp:LinkButton ID="lbtnAdd" runat="server" OnClientClick="javascript:return GoTo('Job_Add.aspx')">添加招聘</asp:LinkButton>
                    <asp:LinkButton ID="lbtnEdit" runat="server" OnClientClick="javascript:return checkEdit('JobID')" OnClick="btnEdit_Click">修改招聘</asp:LinkButton>
                    <asp:LinkButton ID="lbtnDel" runat="server" OnClientClick="javascript:return checkDel('JobID')" OnClick="btnDel_Click">删除招聘</asp:LinkButton>
                    <asp:LinkButton ID="lbtnOpen" runat="server" OnClientClick="javascript:return checkOperate('JobID')" OnClick="btnOpen_Click">批量开放</asp:LinkButton>
                    <asp:LinkButton ID="lbtnClose" runat="server" OnClientClick="javascript:return checkOperate('JobID')" OnClick="btnClose_Click">批量关闭</asp:LinkButton>
                    <asp:LinkButton ID="lbtnTransfer" runat="server" OnClientClick="javascript:return checkOperate('JobID')" OnClick="btnTransfer_Click">转移招聘</asp:LinkButton>
                    <a href="javascript:SearchDialog()">查询招聘</a>
                </td>
            </tr>
        </table>
        <!--操作选项结束-->
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_list" id="TbList">
            <tr class="table_list_title">
                <td style="width: 2%">
                    <input type="checkbox" name="checkbox" value="checkbox" onclick="javascript:checkAll(this,'JobID')" />
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=JobID&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    自动编号<%=GetData.GetOrderSign(strOrderKey, "JobID",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=JobName&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    职位名称<%=GetData.GetOrderSign(strOrderKey, "JobName",strAscDesc1)%>
                </td>
                ...
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=Department&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    招聘部门<%=GetData.GetOrderSign(strOrderKey, "Department",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=JobNum&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    招聘人数<%=GetData.GetOrderSign(strOrderKey, "JobNum",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=WorkPlace&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    工作地点<%=GetData.GetOrderSign(strOrderKey, "WorkPlace",strAscDesc1)%>
                </td>
                <td style="width: 10%" onclick="javascript:GoTo('?OrderKey=EndTime&AscDesc=<%=strAscDesc2 %>&<%=UrlPara %>page=<%=page%>')" title="点击排序">
                    截止日期<%=GetData.GetOrderSign(strOrderKey, "EndTime",strAscDesc1)%>
                </td>
            ...
            <asp:Repeater ID="repList" runat="server">
                <ItemTemplate>
                    <tr class="<%#(Container.ItemIndex+1)%2==0?"table_list_row_alert":"table_list_row_normal" %>">
                        <td style="width: 2%">
                            <input name="JobID" type="checkbox" id="JobID" value="<%#Eval("JobID") %>" onclick="javascript:TrColor2('JobID',<%#Container.ItemIndex+1 %>)" />
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('JobID',<%#Container.ItemIndex+1 %>)">
                            <%#Eval("JobID")%>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('JobID',<%#Container.ItemIndex+1 %>)">
                            <a href="Job_Add.aspx?JobID=<%#Eval("JobID") %>&<%=UrlOrderPara + UrlPara %>page=<%=page %>">
                                <%#Eval("JobName")%>
                            </a>
                        </td>
                        <td style="width: 10%" onclick="javascript:TrColor('JobID',<%#Container.ItemIndex+1 %>)">
